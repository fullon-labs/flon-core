...
The `http_plugin` is a core plugin supported by both `funod` and `fowal`. The plugin is required to enable any RPC API functionality provided by a `funod` or `fowal` instance.
