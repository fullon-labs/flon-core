...
If defaults can be used, then `fucli get info` will output a block that contains a field called `server_version`.  If your `funod` is not using the defaults, then you need to know the URL of the `funod`. In that case, use the following with your `funod` URL:
