...
`trace_api_util` is a command-line interface (CLI) utility that allows node operators to perform low-level tasks associated with the [Trace API Plugin](../01_node/03_plugins/trace_api_plugin/index.md). `trace_api_util` can perform one of the following operations:
