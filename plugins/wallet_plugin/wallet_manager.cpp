   ...
   t->async_wait([t, this](const boost::system::error_code& /*ec*/)
   {
      ...
      if(!ec) {
         if(rc.type() == std::filesystem::file_type::not_found) {
            appbase::app().quit();
            EOS_THROW(wallet_exception, "Lock file removed while wallet progress still running.  Terminating.");
         }
      }
...
void wallet_manager::initialize_lock() {
   //This is technically somewhat racy in here -- if multiple wallet progresses are in this function at once.
   ...
   if(!wallet_dir_lock->try_lock()) {
      ...
      EOS_THROW(wallet_exception, "Failed to lock access to wallet directory; is another wallet progress running?");
   }
