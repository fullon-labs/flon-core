...
void compatible_versions_test()
{
   ...
   if (generate_log) {
      ...
      chain.close();
      ilog("Generate block files: ${log}, ${idx}", ("log", source_log_dir / "blocks.log")("idx", source_log_dir / "blocks.index"));
   }
   ...
   for(std::string version : {"v8"}) // min support version: v8
   {
   ...
   if constexpr (std::is_same_v<TESTER, savanna_tester> && !std::is_same_v<SNAPSHOT_SUITE, threaded_snapshot_suite>) {
      if (save_snapshot)
      {
         ...
         auto latest = SNAPSHOT_SUITE::finalize(latest_writer);
         ilog("Write snapshot to file ${f}", ("f", "snap_" + current_version));
