...
static void create_accounts(validating_tester& chain) {
   chain.produce_block();
   chain.create_accounts({"eosio.msig"_n, token_name});
   chain.produce_block();
...
static asset get_currency_balance(const validating_tester& chain, account_name account) {
   return chain.get_currency_balance(token_name, symbol(SY(4,CUR)), account);
}
...
BOOST_FIXTURE_TEST_CASE( delay_error_create_account, validating_tester_no_disable_deferred_trx) { try {
   ...
   produce_block();
   produce_block(fc::seconds(2) + fc::milliseconds(config::block_interval_ms));
...
BOOST_AUTO_TEST_CASE( link_delay_direct_test ) { try {
   ...
   chain.push_action(token_name, name("issue"), token_name, fc::mutable_variant_object()
           ("to",       token_name)
           ("quantity", "1000000.0000 CUR")
           ("memo", "for stuff")
   );

   auto trace = chain.push_action(token_name, name("transfer"), token_name, fc::mutable_variant_object()
       ("from", token_name)
       ("to", "tester")
   ...
   auto liquid_balance = get_currency_balance(chain, token_name);
   ...
   trace = chain.push_action(token_name, name("transfer"), "tester"_n, fc::mutable_variant_object()
       ("from", "tester")
       ("to", "tester2")
       ("quantity", "1.0000 CUR")
       ("memo", "hi" )
   ...
   liquid_balance = get_currency_balance(chain, token_name);
   BOOST_REQUIRE_EQUAL(asset::from_string("999900.0000 CUR"), liquid_balance);
   liquid_balance = get_currency_balance(chain, "tester"_n);
...
BOOST_AUTO_TEST_CASE( link_delay_direct_parent_permission_test ) { try {
   ...
   chain.produce_block(fc::seconds(14));
...
BOOST_AUTO_TEST_CASE( link_delay_link_change_test ) { try {
   ...
   chain.produce_block();
   chain.produce_block(fc::seconds(7) + fc::milliseconds(config::block_interval_ms));
   ...
   chain.produce_block();
   chain.produce_block(fc::seconds(7) + fc::seconds(config::block_interval_ms));
...
BOOST_AUTO_TEST_CASE( canceldelay_test ) { try {
   ...
   trx.sign(chain.get_private_key("tester"_n, "active"), chain.get_chain_id());
...
BOOST_AUTO_TEST_CASE( canceldelay_test2 ) { try {
...

   {
      ...
      trace = chain.push_action(token_name, name("transfer"), vector<permission_level>{{"tester"_n, "first"_n}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "1.0000 CUR")
          ("memo", "hi" ),
      ...
      // attempt canceldelay with "second" permission for delayed transfer of 1.0000 CUR
      {
         ...
         chain.set_transaction_headers(trx);
         trx.sign(chain.get_private_key("tester"_n, "second"), chain.get_chain_id());
      ...
      chain.produce_block();
      chain.produce_block(fc::seconds(4));
      chain.produce_block();
   ...
   chain.produce_block();
   chain.produce_block(fc::seconds(5));

   ilog("attempting second delayed transfer");
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action(token_name, name("transfer"), vector<permission_level>{{"tester"_n, "second"_n}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "5.0000 CUR")
          ("memo", "hi" ),
      ...
      chain.set_transaction_headers(trx);
      trx.sign(chain.get_private_key("tester"_n, "first"), chain.get_chain_id());
      trace = chain.push_transaction(trx);
   ...
   {
      // this transaction will be delayed 10 blocks
      trace = chain.push_action(token_name, name("transfer"), vector<permission_level>{{"tester"_n, config::owner_name}}, fc::mutable_variant_object()
          ("from", "tester")
          ("to", "tester2")
          ("quantity", "10.0000 CUR")
          ("memo", "hi" ),
      ...
      trx.actions.emplace_back(vector<permission_level>{{"tester"_n, config::owner_name}},
                               chain::canceldelay{{"tester"_n, config::owner_name}, trx_id});
      chain.set_transaction_headers(trx);
      trx.sign(chain.get_private_key("tester"_n, "owner"), chain.get_chain_id());
      trace = chain.push_transaction(trx);
...
BOOST_AUTO_TEST_CASE( max_transaction_delay_execute ) { try {
   ...
   auto trace = chain.push_action(token_name, updateauth::get_name(), tester_account, fc::mutable_variant_object()
                     ("account", "tester")
                     ("permission", "first")
                     ("parent", "active")
   ...
   //check that the delayed msig transaction can be executed after 60 sec
   chain.produce_block();
