...
BOOST_AUTO_TEST_CASE_TEMPLATE( contract_whitelist, T, whitelist_blacklist_validating_testers ) { try {
   ...
   test.chain->set_code("bob"_n, test_contracts::system_token_wasm() );
   test.chain->set_abi("bob"_n, test_contracts::system_token_abi() );

   test.chain->produce_block();

   test.chain->set_code("charlie"_n, test_contracts::system_token_wasm() );
   test.chain->set_abi("charlie"_n, test_contracts::system_token_abi() );
...
BOOST_AUTO_TEST_CASE_TEMPLATE( blacklist_eosio, T, whitelist_blacklist_testers ) { try {
   ...
   tester1.chain->set_code(config::system_account_name, test_contracts::system_token_wasm() );
   tester1.chain->produce_block();
   tester1.shutdown();
...
BOOST_AUTO_TEST_CASE_TEMPLATE( greylist_limit_tests, T, testers ) { try {
   ...
   // TODO: fix me
   // BOOST_REQUIRE_EQUAL( rm.get_account_cpu_limit(user_account).first.max, user_cpu_per_day );
   // BOOST_REQUIRE_EQUAL( rm.get_account_net_limit(user_account).max, user_net_per_day );
   // BOOST_REQUIRE_EQUAL( rm.get_account_cpu_limit(user_account, 1).first.max, user_cpu_per_day );
   // BOOST_REQUIRE_EQUAL( rm.get_account_net_limit(user_account, 1).first.max, user_net_per_day );
   ...
   wdump((rm.get_account_net_limit(user_account)));
   ...
   // TODO: fix me
   // wdump((rm.get_account_net_limit(user_account)));
   // BOOST_REQUIRE( rm.get_account_net_limit(user_account).max > 3*reqauth_net_charge );
   // BOOST_REQUIRE( rm.get_account_net_limit(user_account).max < 4*reqauth_net_charge );
   ...
   // TODO: fix me
      // auto user_elastic_cpu_limit = rm.get_account_cpu_limit(user_account).first.max;
      // auto user_elastic_net_limit = rm.get_account_net_limit(user_account).max;

      // auto user_cpu_res1 = rm.get_account_cpu_limit(user_account, 4);
      // BOOST_REQUIRE_EQUAL( user_cpu_res1.first.max, user_elastic_cpu_limit );
      // BOOST_REQUIRE_EQUAL( user_cpu_res1.second, false );
      // auto user_net_res1 = rm.get_account_net_limit(user_account, 4);
      // BOOST_REQUIRE_EQUAL( user_net_res1.first.max, user_elastic_net_limit );
      // BOOST_REQUIRE_EQUAL( user_net_res1.second, false );

      // auto user_cpu_res2 = rm.get_account_cpu_limit(user_account, 3);
      // BOOST_REQUIRE( user_cpu_res2.first.max < user_elastic_cpu_limit );
      // BOOST_REQUIRE_EQUAL( user_cpu_res2.second, true );
      // auto user_net_res2 = rm.get_account_net_limit(user_account, 3);
