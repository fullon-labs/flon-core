...
// -----------------------------------------------------------------------------------------------------
// Test case demonstrating the weak masking issue (see https://github.com/AntelopeIO/fullon/issues/534)
// Because the issue is fixed in fullon https://github.com/AntelopeIO/fullon/pull/537, test must pass
...
// -----------------------------------------------------------------------------------------------------
// see https://github.com/AntelopeIO/fullon/issues/621 explaining the issue that this test demonstrates.
//
// The fix in https://github.com/AntelopeIO/fullon/issues/534 for the weak masking issue respected a more
...
// quickly enough.
//
// This testcase fails prior to https://github.com/AntelopeIO/fullon/issues/621 being fixed.
// -----------------------------------------------------------------------------------------------------
/* -----------------------------------------------------------------------------------------------------
                                 testcase
                                 --------
...
//
// Then the operator shuts down node and decides to restart from the snapshot on B3.
//
...
//
// Then the operator shuts down node and decides to restart from the snapshot on B6.
//
...
BOOST_FIXTURE_TEST_CASE(validate_qc_requiring_finalizer_policies, savanna_cluster::cluster_t) try {
   ...
   // At this point, in the fullon 1.0.0 implementation (which has the bug described in issue #694),
...
// ensure that this doesn't create a consensus incompatibility with fullon 1.0.0, so the blocks created
// with newer versions remain compatible (and linkable) with blocks by fullon 1.0.0.
//
...
BOOST_FIXTURE_TEST_CASE(verify_block_compatibitity, savanna_cluster::cluster_t) try {
   ...
   if (save_blockchain) {
      ilog("save blockchain data and end.");
...
steps mentioned in comments below refer to issue https://github.com/AntelopeIO/fullon/issues/751
