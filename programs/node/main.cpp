...
int main(int argc, char** argv)
{
   ...
   try {
      ...
      ilog("${name} version ${ver} ${fv}",
            ("name", node::config::node_executable_name)("ver", app->version_string())
            ("fv", app->version_string() == app->full_version_string() ? "" : app->full_version_string()) );
      ilog("${name} using configuration file ${c}", ("name", node::config::node_executable_name)("c", app->full_config_file_path().string()));
      ilog("${name} data directory is ${d}", ("name", node::config::node_executable_name)("d", app->data_dir().string()));
      ::detail::log_non_default_options(app->get_parsed_options());
      app->startup();
