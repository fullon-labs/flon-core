/**
  ...
  @section intro Introduction to client

  `client` is a command line tool that interfaces with the REST api exposed by @ref node. In order to use `client` you will need to
  have a local copy of `node` running and configured to load the 'eosio::chain_api_plugin'.

   client contains documentation for all of its commands. For a list of all commands known to client, simply run it with no arguments:
```
$ ./client
Command Line Interface to EOSIO Client
Usage: programs/client/client [OPTIONS] SUBCOMMAND
...
Options:
  ...
  --wallet-url TEXT=http://localhost:8888/
                              the http URL where wallet progress is running
...
Usage: ./client create SUBCOMMAND
...
Usage: ./client create account [OPTIONS] creator name OwnerKey ActiveKey
...
//copy pasta from program wallet's main.cpp
...
string default_wallet_folder = config::system_account_name.to_string() + "-wallet";
string default_wallet_url = "unix://" + (determine_home_directory() / default_wallet_folder / (string(key_store_executable_name) + ".sock")).string();
string default_eos_wallet_url = "unix://" + (determine_home_directory() / "eosio-wallet" / (string(key_store_executable_name) + ".sock")).string();
...
void print_action( const fc::variant& at ) {
   ...
   /*
   if( code == config::system_account_name.to_string() && func == "setcode" )
      args = args.substr(40)+"...";
...
void ensure_wallet_running(CLI::App* app) {
    if (no_auto_wallet)
        return;
    // get, version, net, convert do not require wallet
    ...
    if (!std::filesystem::exists(binPath)) {
        binPath = parent_path.parent_path() / "wallet"/ key_store_executable_name;
    }
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->callback([this] {
            auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Change to voter.value when client no longer needs to support node versions older than 1.5.0
                               ("limit", 1)
            );
            auto res = result.as<eosio::chain_apis::read_only::get_table_rows_result>();
            // Condition in if statement below can simply be res.rows.empty() when client no longer needs to support node versions older than 1.5.0
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so client can still work with old buggy node versions
                               // Change to newname.value when client no longer needs to support node versions older than 1.5.0
                               ("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
...
struct activate_subcommand {
   ...
   std::string account_str = config::system_account_name.to_string();
   std::string permission_str = config::system_account_name.to_string();
...
void get_account( const string& accountName, const string& coresym, bool json_format ) {
   ...
   if (!json_format) {
      ...
      std::cout << indent << config::eosio_any_name.to_string() << std::endl;
...
//                                ->check(CLI::ExistingFile);
   ...
   // Transfer subcommand
   string con = config::token_account_name.to_string();
   string sender;
   string recipient;
   string amount;
   string memo;
   ...
   stopKeosd->callback([] {
      ...
      if ( !v.is_object() || v.get_object().size() != 0 ) { //on success wallet responds with empty object
   ...
   propose_action->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, config::msig_account_name, "propose"_n, variant_to_bin( config::msig_account_name, "propose"_n, args ) }}, signing_keys_opt.get_keys());
   });
   ...
   review->callback([&] {
      ...
      // Condition in if statement below can simply be rows.empty() when client no longer needs to support node versions older than 1.5.0
         ...
         if( new_multisig ) {
            for( auto& a : provided_approvers ) {
               const auto result4 = call(get_table_func, fc::mutable_variant_object("json", true)
                                          ("code", config::msig_account_name)
                                          ("scope", config::msig_account_name)
                                          ("table", "invals")
                                          ("table_key", "")
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, config::msig_account_name, name(action), variant_to_bin( config::msig_account_name, name(action), args ) }}, signing_keys_opt.get_keys());
   };
   ...
   invalidate->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, config::msig_account_name, "invalidate"_n, variant_to_bin( config::msig_account_name, "invalidate"_n, args ) }}, signing_keys_opt.get_keys());
   });
   ...
   cancel->callback([&]() {
      ...
      send_actions({chain::action{accountPermissions, config::msig_account_name, "cancel"_n, variant_to_bin( config::msig_account_name, "cancel"_n, args ) }}, signing_keys_opt.get_keys());
      }
   );
   ...
   exec->callback([&] {
      ...
      send_actions({chain::action{accountPermissions, config::msig_account_name, "exec"_n, variant_to_bin( config::msig_account_name, "exec"_n, args ) }}, signing_keys_opt.get_keys());
      }
   );
   ...
   // wrap exec
   string wrap_con = config::wrap_account_name.to_string();
   executer = "";
   string trx_to_exec;
   ...
   // system subcommand
   auto system = app.add_subcommand("system", localized("Send system contract action to the blockchain."));
