...
#define SET_SYS_APP_HANDLER( action ) \
   set_apply_handler( config::system_account_name, config::system_account_name, action_name(#action), \
                      &BOOST_PP_CAT(apply_eosio_, action) )

      SET_SYS_APP_HANDLER( newaccount );
      SET_SYS_APP_HANDLER( setcode );
      SET_SYS_APP_HANDLER( setabi );
      SET_SYS_APP_HANDLER( updateauth );
      SET_SYS_APP_HANDLER( deleteauth );
      SET_SYS_APP_HANDLER( linkauth );
      SET_SYS_APP_HANDLER( unlinkauth );

      SET_SYS_APP_HANDLER( canceldelay );
   }
   ...
   void init(startup_t startup) {
      ...
      auto finish_init = [&](auto& forkdb) {
         if( read_mode != db_read_mode::IRREVERSIBLE ) {
            ...
            // is a bit of a hack for fullon 1.0.0 until we can add a proper pause-at-block (issue #570) which could
   ...
   block_state_pair read_from_snapshot( const snapshot_reader_ptr& snapshot, uint32_t blog_start, uint32_t blog_end ) {
      ...
      if (header.version >= v8::minimum_version) {
         // loading a snapshot saved by fullon 1.0.1 and above.
         // ---------------------------------------------------
         ...
         // snapshot created with fullon 1.0.0, which was very soon superseded by fullon 1.0.1
         // and a new snapshot format.
         // ----------------------------------------------------------------------------------
         EOS_THROW(snapshot_exception, "v7 snapshots are not supported anymore in fullon 1.0.1 and above");
      } else {
...
template<>
void controller_impl::on_activation<builtin_protocol_feature_t::action_data_to_json>() {
   db.modify( db.get<protocol_state_object>(), [&]( auto& ps ) {
