...
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   if( !creator.is_privileged() ) {
      EOS_ASSERT( name_str.find( config::system_name_prefix_str ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with '${p}'", ("p", config::system_name_prefix_str) );
   }
...
void apply_eosio_updateauth(apply_context& context) {
   ...
   EOS_ASSERT( update.permission.to_string().find( config::system_name_prefix_str ) != 0, action_validate_exception,
               "Permission names that start with '${p}' are reserved", ("p", config::system_name_prefix_str) );
