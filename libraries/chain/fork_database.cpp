...
// ------------------ fork_database -------------------------
   ...
   void fork_database::close() {
      ...
      // version == 1 -> legacy
      // version == 2 -> fullon 1.0.0
      //                 (two possible fork_db, one containing `block_state_legacy`, one containing `block_state`)
      //                  unsupported by fullon 1.0.1 and above
      // version == 3 -> fullon 1.0.1 updated block_header_state (core with policy gen #)
      //                 (two possible fork_db, one containing `block_state_legacy`, one containing `block_state`)
      // ---------------------------------------------------------------------------------------------------------
      ...
      if( std::filesystem::exists( fork_db_file ) ) {
         try {
            ...
            EOS_ASSERT( version != 2, fork_database_exception,
                        "Version 2 of fork_database (created by fullon 1.0.0) is not supported" );
