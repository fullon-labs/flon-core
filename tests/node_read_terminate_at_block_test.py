...
###############################################################
# node_read_terminate_at_block_test
#
...
def executeSnapshotBlocklogTest(cluster, testNodeId, resultMsgs, nodeArgs, termAtBlock):
    ...
    errFileName=f"{cluster.nodeLogPath}/node_{str(testNodeId).zfill(2)}/stderr.txt"
...
try:
    ...
    specificNodeArgs.update(regularNodeArgs)
    specificNodeArgs.update(replayNodeArgs)
    ...
    for nodeId, nodeArgs in regularNodeArgs.items():
        success = executeTest(
            cluster,
            nodeId,
            nodeArgs,
    ...
    if success:
        for nodeId, nodeArgs in replayNodeArgs.items():
    ...
    if success:
        for nodeId, nodeArgs in {**regularNodeArgs, **replayNodeArgs}.items():
