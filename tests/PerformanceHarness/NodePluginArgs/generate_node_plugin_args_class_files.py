...
to encapsulate the configurations options available for each plugin as currently documented in node's --help command.

It makes use of the compiled node program and runs the --help command, capturing the output.
...
The script then uses the parsed output to generate *PluginArgs.py scripts, placing them in the NodePluginArgs directory.
...
Each *PluginArgs.py file contains one dataclass that captures the available configuration options for that plugin via node command line.

Each config option is represented by 3 member variables, for example:
1) blocksDir: str=None
    --This is the field that will be populated when the dataclass is used by other scripts to configure node
2) _blocksDirNodeDefault: str='"blocks"'
    --This field captures the default value in the node output.  This will be compared against the first field to see if the configuration
    option will be required on the command line to override the default value when running node.
3) _blocksDirNodeArg: str="--blocks-dir"
...
2) __str__
    -- Provides the command line argument string for the current configuration to pass to node
...
- There are some hardcoded edge cases when trying to determine the types associated with certain default argument parameters.
  These may need to be updated to account for new/different options as they are added/removed/modified by node

Note:
- To help with maintainability the validate_node_plugin_args.py test script is provided which validates the current
  *PluginArgs dataclass configuration objects against the current node --help output to notify developers when
...
def main():
    ...
    def writeDataclass(plugin:str, dataFieldDict:dict, pluginOptsDict:dict):
        ...
        with open(pluginArgsFile, 'w') as dataclassFile:
            ...
            dataclassFile.write(f"This file/class was generated by generate_node_plugin_args_class_files.py\n")
            dataclassFile.write(f"\"\"\"\n\n")
            ...
            for key, value in pluginOptsDict[plugin].items():
                ...
                if match is not None:
                    ...
                    try:
                        ...
                        dataclassFile.write(f"    _{newKey}nodeDefault: int={numVal}\n")
                        dataclassFile.write(f"    _{newKey}nodeArg: str=\"{key}\"\n")
                    except ValueError:
                        strValue = str(value)
                        quote = "\'" if re.search("\"", strValue) else "\""
                        dataclassFile.write(f"    {newKey}: str=None\n")
                        dataclassFile.write(f"    _{newKey}nodeDefault: str={quote}{strValue}{quote}\n")
                        dataclassFile.write(f"    _{newKey}nodeArg: str=\"{key}\"\n")
                else:
                    if re.search("deepmind", newKey, re.IGNORECASE) or \
                       ...
                       re.search("disable", newKey, re.IGNORECASE):
                        ...
                        dataclassFile.write(f"    _{newKey}nodeDefault: bool=False\n")
                        dataclassFile.write(f"    _{newKey}nodeArg: str=\"{key}\"\n")
                        ...
                        dataclassFile.write(f"    {newKey}: int=None\n")
                        dataclassFile.write(f"    _{newKey}nodeDefault: int=None\n")
                        dataclassFile.write(f"    _{newKey}nodeArg: str=\"{key}\"\n")
                    else:
                        dataclassFile.write(f"    {newKey}: str=None\n")
                        dataclassFile.write(f"    _{newKey}nodeDefault: str=None\n")
                        dataclassFile.write(f"    _{newKey}nodeArg: str=\"{key}\"\n")
...
def main():\n\
    ...
    print(pluginArgs.supportedNodeArgs())\n\
    exit(0)\n\n\
