...
def test_create_key_to_console():
    rslts = Utils.processFullOnUtilCmd("bls create key --to-console", "create key to console", silentErrors=False)
    check_create_key_results(rslts)

def test_create_key_to_file():
    tmp_file = "tmp_key_file_dlkdx1x56pjy"
    Utils.processFullOnUtilCmd("bls create key --file {}".format(tmp_file), "create key to file", silentErrors=False)
...
def test_create_pop_from_command_line():
    ...
    rslts = Utils.processFullOnUtilCmd("bls create pop --private-key {}".format(private_key), "create pop from command line", silentErrors=False)
    results = get_results(rslts)
...
def test_create_pop_from_file():
    ...
    rslts = Utils.processFullOnUtilCmd("bls create pop --file {}".format(private_key_file), "create pop from command line", silentErrors=False)
...
def test_create_key_error_handling():
    # should fail with missing arguments (processFullOnUtilCmd returning None)
    assert Utils.processFullOnUtilCmd("bls create key", "missing arguments") == None

    # should fail when both arguments are present
    assert Utils.processFullOnUtilCmd("bls create key --file out_file --to-console", "conflicting arguments") == None

def test_create_pop_error_handling():
    # should fail with missing arguments (processFullOnUtilCmd returning None)
    assert Utils.processFullOnUtilCmd("bls create pop", "missing arguments") == None

    # should fail when both arguments are present
    assert Utils.processFullOnUtilCmd("bls create pop --file private_key_file --private-key", "conflicting arguments") == None
