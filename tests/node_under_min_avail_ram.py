...
###############################################################
# node_under_min_avail_ram
#
# Sets up 4 producing nodes using --chain-state-db-guard-size-mb and --chain-state-db-size-mb to verify that node will
...
try:
    ...
    maxRAMValue=1010
    extraNodeArgs=" %s %d %s %d  --http-max-response-time-ms 990000 " % (minRAMFlag, minRAMValue, maxRAMFlag, maxRAMValue)
    if cluster.launch(onlyBios=False, pnodes=pNodes, totalNodes=totalNodes, totalProducers=totalNodes, activateIF=activateIF, extraNodeArgs=extraNodeArgs) is False:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.sysAccount])
        ...
        for fromIndex in range(namedAccounts.numAccounts):
            ...
            try:
                ...
                if trans is None or not trans[0]:
                    timeOutCount+=1
                    if timeOutCount>=3:
                        Print("Failed to push create action to system contract for %d consecutive times, looks like node already exited." % (timeOutCount))
                        keepProcessing=False
                        break

                    Print("Failed to push create action to system contract. sleep for 5 seconds")
    ...
    # min and max are subjective, just assigned to make sure that many small changes in node don't
    ...
    while keepProcessing and count < 40:
        ...
        for fromIndexOffset in range(namedAccounts.numAccounts):
            ...
            try:
                trans=nodes[count % numNodes].pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Print("Failed to push create action to system contract. sleep for 60 seconds")
                    time.sleep(60)
                time.sleep(1)
            except TypeError as ex:
