...
getHeadBlockNum()
{
  INFO="$(./bin/fucli get info)"
  verifyErrorCode "fucli get info"
...
# stand up node cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --node \"--plugin eosio::wallet_api_plugin\" $launcherOpts
programs/eosio-launcher/eosio-launcher --node "--plugin eosio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(./bin/fucli --port $port2 get block 1)"
    verifyErrorCode "fucli get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(./bin/fucli create key)"
verifyErrorCode "fucli create key"
...
PASSWORD_INITA="$(./bin/fucli wallet create --name inita)"
verifyErrorCode "fucli wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
./bin/fucli wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "fucli wallet import"
./bin/fucli wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "fucli wallet import"
./bin/fucli wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "fucli wallet import"
./bin/fucli wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "fucli wallet import"
...
ACCOUNT_INFO="$(./bin/fucli create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "fucli create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(./bin/fucli get account testera)"
verifyErrorCode "fucli get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(./bin/fucli transfer inita testera 975321 "test transfer")"
    verifyErrorCode "fucli transfer"
   ...
   TRANS_INFO="$(./bin/fucli --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "fucli get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
