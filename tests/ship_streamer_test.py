...
#   error. The fork generated by node should be sent to the ship_streamer so it is able to correctly observe the
#   fork.
#
###############################################################
...
try:
    ...
    specificExtraNodeArgs[shipNodeNum]="--plugin eosio::state_history_plugin --trace-history --chain-state-history --state-history-stride 200 --plugin eosio::net_api_plugin --plugin eosio::producer_api_plugin "
    if args.finality_data_history:
        specificExtraNodeArgs[shipNodeNum]+=" --finality-data-history"
    ...
    if cluster.launch(topo="./tests/bridge_for_fork_test_shape.json", pnodes=totalProducerNodes, loadSystemContract=False,
                      ...
                      extraNodeArgs=extraNodeArgs, specificExtraNodeArgs=specificExtraNodeArgs) is False:
    ...
    jumboAcc.ownerPublicKey = Utils.SysAccountPubKeyDefault
    jumboAcc.activePublicKey = Utils.SysAccountPubKeyDefault
    ...
    Print("Shutdown state_history_plugin node after replay")
    shipNode.kill(signal.SIGTERM)
