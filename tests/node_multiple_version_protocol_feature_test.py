...
#
# Test for verifying that older versions of node can work with newer versions of node.
#
###############################################################
...
def restartNode(node: Node, chainArg=None, addSwapFlags=None, nodePath=None):
    if not node.killed:
        node.kill(signal.SIGTERM)
    isRelaunchSuccess = node.relaunch(chainArg, addSwapFlags=addSwapFlags,
                                      timeout=5, nodePath=nodePath)
...
try:
    ...
    # Finally, when we restart the 4th node with the version of node that supports protocol feature,
    ...
    oldNode.relaunch(chainArg="--import-reversible-blocks {}".format(portableRevBlkPath), timeout=1, nodePath=Utils.NodeServerPath)
    os.remove(portableRevBlkPath)

    restartNode(oldNode, chainArg="--replay", nodePath=Utils.NodeServerPath)
